...
module make_case(w,d,h,bt,lt) {
    ...
    w = w + 10;
    d = d + 10;
    // Now we just subtract the shape we have created in the four corners
    translate([-5, 0, 0]){ color(CASEcolor) difference() {
        ...
        translate(v = [box_thickness+5, box_thickness+5, -2]) cube([inside_w, inside_l-10, box_h+4], center = false);
    }}
    if (!bottomframe) {
        if (!bottominlet) {
